...
data class Video(val id: String, var title: String, val duration: Long=0, val folderName: String, val folderId: String,  val size: String,
            var path: String, var artUri: Uri)

data class Folder(val id: String, val folderName: String, val folderPath: String)
    ...
    if(cursor != null){
        if(cursor.moveToNext()){
            do{
                ...
                val folderPathC = File(pathC).parent.toString()
                //Toast.makeText(this, folderPathC, Toast.LENGTH_SHORT).show()
                ...
                try {
                    ...
                    // for adding folders
                    if(!tempFolderList.contains(folderPathC)){
                        tempFolderList.add(folderPathC)
                        MainActivity.folderList.add(Folder(id=folderIdC, folderName = folderC, folderPath=folderPathC))
                    }
                }catch (e: Exception){}
